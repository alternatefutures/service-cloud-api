...
export class ChatServer {
   ...
   */
  private async handleAuthenticate(ws: WebSocket, payload: WSAuthenticatePayload): Promise<void> {
    try {
      // Verify JWT token
      const decoded = jwt.verify(payload.token, this.jwtSecret) as { userId: string };
   ...
   * Handle send message
   */
  private async handleSendMessage(ws: WebSocket, payload: WSSendMessagePayload): Promise<void> {
    ...
    try {
      ...
      const messages = await this.messageService.getChatMessages(payload.chatId, 20);
