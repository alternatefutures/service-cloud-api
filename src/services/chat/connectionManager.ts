...
export class ConnectionManager {
  ...
  addConnection(ws: WebSocket, userId: string, chatId?: string): void {
    ...
    console.log(`✅ Connection added for user ${userId} in chat ${chatId || 'none'}`);
  }
   ...
   */
  broadcastToChat(chatId: string, message: WSMessage, except?: WebSocket): void {
    const sockets = this.chatConnections.get(chatId);
    if (!sockets) return;

    const messageStr = JSON.stringify(message);
    sockets.forEach((ws) => {
      if (ws !== except && ws.readyState === 1) { // WebSocket.OPEN
        ws.send(messageStr);
      }
    });
  }
