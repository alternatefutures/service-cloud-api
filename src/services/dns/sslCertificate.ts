 ...
 */
export async function createAcmeClient(config: AcmeConfig): Promise<acme.Client> {
  const directoryUrl = config.directoryUrl || acme.directory.letsencrypt.production;
...
export async function requestSslCertificate(
  ...
  try {
    ...
    throw new Error(`SSL certificate request failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function requestWildcardCertificate(
  ...
  try {
    ...
    throw new Error(`Wildcard certificate request failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
  ...
  for (const domain of domains) {
    if (domain.sslExpiresAt && shouldRenewCertificate(domain.sslExpiresAt)) {
      try {
        ...
        console.error(`Failed to renew certificate for ${domain.hostname}:`, error);
        await prisma.domain.update({
          where: { id: domain.id },
...
async function storeHttpChallenge(domain: string, token: string, keyAuth: string): Promise<void> {
...
async function removeHttpChallenge(domain: string, token: string): Promise<void> {
  console.log(`Remove HTTP challenge for ${domain}: ${token}`);
  // TODO: Implement challenge removal
}

async function storeDnsChallenge(domain: string, token: string, dnsRecord: string): Promise<void> {
...
async function removeDnsChallenge(domain: string, token: string): Promise<void> {
