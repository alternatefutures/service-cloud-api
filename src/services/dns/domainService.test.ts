...
describe('Domain Service', () => {
  ...
  describe('createCustomDomain', () => {
    ...
    it('should throw error if domain already exists', async () => {
      ...
      await expect(createCustomDomain(input, mockPrisma)).rejects.toThrow('already registered');
    });
  ...
  describe('getVerificationInstructions', () => {
    it('should return TXT record instructions', async () => {
      ...
      const instructions = await getVerificationInstructions('domain-123', mockPrisma);
    ...
    it('should throw error if domain not found', async () => {
      ...
      await expect(getVerificationInstructions('nonexistent', mockPrisma)).rejects.toThrow('not found');
    });
  });
  ...
  describe('provisionSslCertificate', () => {
    it('should provision SSL for verified domain', async () => {
      ...
      await provisionSslCertificate('domain-123', 'admin@example.com', mockPrisma);
    ...
    it('should throw error if domain not verified', async () => {
      ...
      await expect(provisionSslCertificate('domain-123', 'admin@example.com', mockPrisma)).rejects.toThrow('must be verified');
    });
    ...
    it('should update status to FAILED on error', async () => {
      ...
      await expect(provisionSslCertificate('domain-123', 'admin@example.com', mockPrisma)).rejects.toThrow();
  ...
  describe('setPrimaryDomain', () => {
    ...
    it('should throw error if domain not verified', async () => {
      ...
      await expect(setPrimaryDomain('site-123', 'domain-123', mockPrisma)).rejects.toThrow('verified domains');
    });
    ...
    it('should throw error if domain belongs to different site', async () => {
      ...
      await expect(setPrimaryDomain('site-123', 'domain-123', mockPrisma)).rejects.toThrow('does not belong');
    });
  });
  ...
  describe('removeCustomDomain', () => {
    ...
    it('should throw error if removing primary domain', async () => {
      ...
      await expect(removeCustomDomain('domain-123', mockPrisma)).rejects.toThrow('Cannot remove primary domain');
    });
  });
