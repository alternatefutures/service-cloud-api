...
describe('SubscriptionHealthMonitor', () => {
  ...
  describe('Health Checks', () => {
    ...
    it('should report unhealthy status with high error rate', () => {
      ...
      expect(health.alerts).toContain('High error rate: 11 errors in the last minute');
    });
    ...
    it('should report degraded status with moderate error rate', () => {
      ...
      expect(health.alerts).toContain('Elevated error rate: 6 errors in the last minute');
    });
    ...
    it('should alert on stale events with active subscriptions', () => {
      ...
      const health = subscriptionHealthMonitor.performHealthCheck();
      expect(health.status).toBe('degraded');
      expect(health.alerts.some(alert => alert.includes('No events emitted'))).toBe(true);
    });
    ...
    it('should alert on possible subscription leak', () => {
      ...
      const health = subscriptionHealthMonitor.performHealthCheck();
      expect(health.status).toBe('unhealthy');
      expect(health.alerts.some(alert => alert.includes('Possible subscription leak'))).toBe(true);
    });
    ...
    it('should alert on high number of subscriptions', () => {
      ...
      const health = subscriptionHealthMonitor.performHealthCheck();
      expect(health.status).toBe('degraded');
      expect(health.alerts.some(alert => alert.includes('High number of active subscriptions'))).toBe(true);
    });
  });
