...
describe('RouteMatcher', () => {
  ...
  describe('multiple route scenarios', () => {
    it('should handle complex routing setup', () => {
      ...
      // Auth wildcard
      expect(matcher.match('/api/auth/logout', routes)?.target).toBe(
        'https://auth.example.com'
      )

      // Users wildcard
      expect(matcher.match('/api/users/123', routes)?.target).toBe(
        'https://users.example.com'
      )

      // API fallback
      expect(matcher.match('/api/unknown', routes)?.target).toBe(
        'https://api.example.com'
      )

      // Root fallback
      expect(matcher.match('/homepage', routes)?.target).toBe(
        'https://default.example.com'
      )
    })
  })
