...
const FALLBACK_INSTANCE_TYPES: PhalaInstanceType[] = [
  { id: 'tdx.small',    name: 'small',    vcpu: 1,   memory_mb: 2048,    hourly_rate: '0.058000', requires_gpu: false, family: 'cpu' },
  { id: 'tdx.medium',   name: 'medium',   vcpu: 2,   memory_mb: 4096,    hourly_rate: '0.116000', requires_gpu: false, family: 'cpu' },
  { id: 'tdx.large',    name: 'large',    vcpu: 4,   memory_mb: 8192,    hourly_rate: '0.232000', requires_gpu: false, family: 'cpu' },
  { id: 'tdx.xlarge',   name: 'xlarge',   vcpu: 8,   memory_mb: 16384,   hourly_rate: '0.464000', requires_gpu: false, family: 'cpu' },
  { id: 'h100.small',   name: 'H100',     vcpu: 16,  memory_mb: 131072,  hourly_rate: '2.800000', requires_gpu: true,  family: 'gpu' },
  { id: 'h200.small',   name: 'H200',     vcpu: 24,  memory_mb: 196608,  hourly_rate: '3.500000', requires_gpu: true,  family: 'gpu' },
  { id: 'h200.8x.large',name: 'H200 x8', vcpu: 192, memory_mb: 1572864, hourly_rate: '23.040000',requires_gpu: true,  family: 'gpu' },
  { id: 'b200.small',   name: 'B200',     vcpu: 32,  memory_mb: 262144,  hourly_rate: '4.200000', requires_gpu: true,  family: 'gpu' },
]
...
function compareByCostThenSize(a: PhalaInstanceType, b: PhalaInstanceType): number {
...
export async function listPhalaInstanceTypes(): Promise<PhalaInstanceType[]> {
  if (cachedInstanceTypes && Date.now() - cachedInstanceTypes.fetchedAt < INSTANCE_TYPES_TTL_MS) {
  ...
  try {
    ...
    log.warn({ err: (err as Error).message }, 'Failed to fetch live Phala instance types, using fallback catalog')
  }
...
export async function resolvePhalaInstanceType(
  ...
  if (requestedGpu) {
    ...
    const requiredUnits = requestedGpu.units || 1
    ...
    // Filter by GPU count — instance must provide at least the requested units
    gpuTypes = gpuTypes.filter((type) => inferGpuCountFromId(type.id) >= requiredUnits)

    if (gpuTypes.length === 0) {
      throw new Error(
        normalizedModel
          ? `Requested Phala GPU "${requestedGpu.model}" x${requiredUnits} is not currently available`
          : `No Phala GPU instance with ${requiredUnits}+ GPUs is currently available`
      )
    }
    ...
    return {
      cvmSize: selected.id,
      gpuModel: requestedGpu.model ?? inferGpuModelFromInstanceType(selected.id),
  ...
  const cpuCandidates = types
    ...
    .filter((type) => type.vcpu >= requiredCpu && type.memory_mb >= requiredMemoryMb)
