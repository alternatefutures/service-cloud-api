...
describe('FilecoinStorageService', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should throw error when API key is missing', () => {
      ...
      expect(() => new FilecoinStorageService()).toThrow('Lighthouse API key not configured');
    });
  });
  ...
  describe('upload', () => {
    ...
    it('should handle upload errors', async () => {
      ...
      vi.mocked(lighthouse.upload).mockRejectedValueOnce(new Error('Upload failed'));

      const service = new FilecoinStorageService();

      await expect(service.upload(Buffer.from('test'), 'test.txt')).rejects.toThrow(
        'Filecoin upload failed: Upload failed'
      );
    });
    ...
    it('should handle unknown errors', async () => {
      ...
      await expect(service.upload(Buffer.from('test'), 'test.txt')).rejects.toThrow(
        'Filecoin upload failed: Unknown error'
      );
    });
  ...
  describe('uploadDirectory', () => {
    ...
    it('should handle directory upload errors', async () => {
      const lighthouse = (await import('@lighthouse-web3/sdk')).default;
      vi.mocked(lighthouse.upload).mockRejectedValueOnce(new Error('Directory upload failed'));
  ...
  describe('getUploadStatus', () => {
    ...
    it('should handle getUploadStatus errors', async () => {
      const lighthouse = (await import('@lighthouse-web3/sdk')).default;
      vi.mocked(lighthouse.getUploads).mockRejectedValueOnce(new Error('Failed to get status'));
