...
export class ArweaveStorageService implements StorageService {
  ...
  private async initialize() {
    ...
    try {
      ...
      const jwk = typeof this.privateKey === 'string'
      ...
      throw new Error(`Failed to initialize Arweave client: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async uploadDirectory(dirPath: string): Promise<UploadResult> {
    ...
    try {
      ...
      throw new Error(`Arweave directory upload failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
