...
describe('ArweaveStorageService', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should throw error when private key is missing', async () => {
      ...
      ).rejects.toThrow('Arweave private key not configured')
    })
    ...
    it('should only initialize once', async () => {
      ...
      const firstCallCount = vi.mocked(TurboFactory.authenticated).mock.calls
        .length

      // Second upload should not initialize again
      await service.upload(Buffer.from('test2'), 'test2.txt')

      const secondCallCount = vi.mocked(TurboFactory.authenticated).mock.calls
        .length
  ...
  describe('upload', () => {
    ...
    it('should handle invalid JWK format', async () => {
      ...
      await expect(
        service.upload(Buffer.from('test'), 'test.txt')
      ).rejects.toThrow('Failed to initialize Arweave client')
    })

    it('should handle upload errors', async () => {
      const { TurboFactory } = await import('@ardrive/turbo-sdk')
      const mockTurbo = await TurboFactory.authenticated({
        privateKey: {} as any,
      })
      vi.mocked(mockTurbo.uploadFile).mockRejectedValueOnce(
      ...
      await expect(
        service.upload(Buffer.from('test'), 'test.txt')
      ).rejects.toThrow('Arweave upload failed: Upload failed')
    })
    ...
    it('should handle unknown upload errors', async () => {
      ...
      await expect(
        service.upload(Buffer.from('test'), 'test.txt')
      ).rejects.toThrow('Arweave upload failed: Unknown error')
    })
  })
  ...
  describe('uploadDirectory', () => {
    ...
    it('should handle missing directory', async () => {
      ...
      await expect(
        service.uploadDirectory('/non-existent-directory')
