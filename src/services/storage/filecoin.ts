...
export class FilecoinStorageService implements StorageService {
  ...
  async upload(data: Buffer | string, filename: string): Promise<UploadResult> {
    try {
      ...
      throw new Error(
        `Filecoin upload failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  async uploadDirectory(dirPath: string): Promise<UploadResult> {
    try {
      ...
      throw new Error(
        `Filecoin directory upload failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  async getUploadStatus(cid: string): Promise<any> {
    try {
      ...
      throw new Error(
        `Failed to get upload status: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
}
