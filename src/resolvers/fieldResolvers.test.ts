...
describe('Field Resolvers', () => {
  ...
  describe('User field resolvers', () => {
    it('should resolve user projects', async () => {
      ...
      vi.mocked(mockContext.prisma.project.findMany).mockResolvedValue(mockProjects);

      const result = await resolvers.User.projects({ id: 'user-123' }, {}, mockContext);
  ...
  describe('Project field resolvers', () => {
    ...
    it('should resolve project sites', async () => {
      ...
      const result = await resolvers.Project.sites({ id: 'project-123' }, {}, mockContext);
    ...
    it('should resolve project functions', async () => {
      ...
      vi.mocked(mockContext.prisma.aFFunction.findMany).mockResolvedValue(mockFunctions);

      const result = await resolvers.Project.functions({ id: 'project-123' }, {}, mockContext);
  ...
  describe('Site field resolvers', () => {
    it('should resolve site project', async () => {
      ...
      vi.mocked(mockContext.prisma.project.findUnique).mockResolvedValue(mockProject);
    ...
    it('should resolve site deployments', async () => {
      ...
      vi.mocked(mockContext.prisma.deployment.findMany).mockResolvedValue(mockDeployments);

      const result = await resolvers.Site.deployments({ id: 'site-123' }, {}, mockContext);
    ...
    it('should resolve site domains', async () => {
      ...
      const result = await resolvers.Site.domains({ id: 'site-123' }, {}, mockContext);
  ...
  describe('AFFunction field resolvers', () => {
    ...
    it('should resolve function current deployment', async () => {
      ...
      vi.mocked(mockContext.prisma.aFFunctionDeployment.findUnique).mockResolvedValue(
        mockDeployment
      );
      ...
      expect(mockContext.prisma.aFFunctionDeployment.findUnique).toHaveBeenCalledWith({
        where: { id: 'deploy-123' },
      });
    });
    ...
    it('should return null when currentDeploymentId is not set', async () => {
      ...
      expect(mockContext.prisma.aFFunctionDeployment.findUnique).not.toHaveBeenCalled();
    });
    ...
    it('should resolve function deployments', async () => {
      ...
      vi.mocked(mockContext.prisma.aFFunctionDeployment.findMany).mockResolvedValue(
        mockDeployments
      );
      ...
      expect(result).toEqual(mockDeployments);
      expect(mockContext.prisma.aFFunctionDeployment.findMany).toHaveBeenCalledWith({
  ...
  describe('AFFunctionDeployment field resolvers', () => {
    it('should resolve deployment function', async () => {
      ...
      vi.mocked(mockContext.prisma.aFFunction.findUnique).mockResolvedValue(mockFunction);
