  ...
  describe('Queries', () => {
    describe('domain', () => {
      it('should return domain for authorized user', async () => {
        ...
        mockContext.prisma.domain.findUnique = vi
          .fn()
          .mockResolvedValue(mockDomain)

        const result = await domainQueries.domain(
          {},
          { id: 'domain-123' },
          mockContext
        )
    ...
    describe('domainVerificationInstructions', () => {
      it('should return verification instructions for authorized user', async () => {
        ...
        mockContext.prisma.domain.findUnique = vi
          .fn()
          .mockResolvedValue(mockDomain)
        vi.mocked(domainService.getVerificationInstructions).mockResolvedValue(
          mockInstructions
        )
  ...
  describe('Mutations', () => {
    describe('createDomain', () => {
      it('should create domain for authorized user', async () => {
        ...
        mockContext.prisma.site.findUnique = vi.fn().mockResolvedValue(mockSite)
        vi.mocked(domainService.createCustomDomain).mockResolvedValue(
          mockDomain as any
        )
    ...
    describe('verifyDomain', () => {
      it('should verify domain for authorized user', async () => {
        ...
        const result = await domainMutations.verifyDomain(
          {},
          { domainId: 'domain-123' },
          mockContext
        )
    ...
    describe('deleteDomain', () => {
      it('should delete domain for authorized user', async () => {
        ...
        const result = await domainMutations.deleteDomain(
          {},
          { id: 'domain-123' },
          mockContext
        )
