...
async function runTest() {
  ...
  try {
    ...
    const wsToken = jwt.sign({ userId: user.id }, JWT_SECRET, { expiresIn: '1h' });
    ...
    await new Promise((resolve) => {
      ...
      ws.on('message', (data) => {
        ...
        if (message.type === 'message_received' && !receivedUserMessage) {
          console.log('   ✅ User message received:', message.payload.content.substring(0, 50));
          receivedUserMessage = true;
        }

        if (message.type === 'agent_response' && !receivedAgentResponse) {
          console.log('   ✅ Agent response received:', message.payload.content.substring(0, 50));
    ...
    messages.forEach((msg, index) => {
      ...
      console.log(`      ${index + 1}. [${msg.role}] ${sender}: ${msg.content.substring(0, 60)}...`);
    });
